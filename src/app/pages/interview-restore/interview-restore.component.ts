      ...
      <div class="max-w-2xl mx-auto text-center w-full animate-slide-up">
        ...
        <div 
          class="glassmorphism rounded-3xl p-6 sm:p-10 shadow-xl max-w-xl mx-auto w-full">
            ...
            <!-- Error Message -->
            @if (errorMessage()) {
            <div class="mt-6 text-[hsl(var(--color-error))] text-sm bg-[hsl(var(--color-error))/0.1] py-2 px-4 rounded-lg inline-block">
                {{ errorMessage() }}
            </div>
            }
            ...
            <!-- Loading State -->
            @if (isProcessing()) {
            <div class="mt-8 flex items-center justify-center gap-3 text-[hsl(var(--color-primary))]">
